...
class _CalendarBookingScreenState extends State<CalendarBookingScreen> {
  ...
  bool _returnReload = false; // Prevent double reload
  final Map<DateTime, bool> _loadedDates = {}; // Track loaded dates
  ...
  Future<void> loadSchedules() async {
    ...
    if (_selectedDate != null) {
      await refreshSchedulesForSelectedDate();
    }
  }
  ...
  Future<void> refreshSchedulesForSelectedDate() async {
    ...
    _loadedDates[_selectedDate!] = true;
  }

  void _onViewChanged(DateRangePickerViewChangedArgs args) async {
    if (_returnReload) return; // skip reload if coming back from ScheduleListScreen
 ...
 void _onDateSelected(DateRangePickerSelectionChangedArgs args) async {
  if (args.value is DateTime) {
    ...
    // Only refresh schedules if NOT returning from ScheduleListScreen
    if (!_returnReload) {
      await refreshSchedulesForSelectedDate();
    }
  }
  ...
  /// Call this from parent screen after returning from ScheduleListScreen
 Future<void> refreshAfterReturn() async {
  if (_selectedDate == null) return;

  _returnReload = true;
  await refreshSchedulesForSelectedDate(); // Refresh **once**
  _returnReload = false;
}
